...
router.get("/installment-cars", authenticateJWT, async (req, res) => {
  try {
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      const planIndex = car.installmentPlans.findIndex(
      ...
      if (planIndex === -1) {
        return res
          .status(404)
          .json({ error: "Installment plan not found for this car." });
      }
